...
from gguf_export import export_gguf_model, export_multiple_quantizations
from hf_upload import upload_to_huggingface, check_hf_auth
...
def save_medical_model(model, tokenizer):
    ...
    print("\n🏥 Medical Emergency AI Model Ready for Further Processing!")
    print("Use separate export/upload scripts for deployment:")
    print("   - Run gguf_export.py for mobile deployment")
    print("   - Run hf_upload.py for HuggingFace Hub upload")
...
def main():
    ...
    try:
        ...
        # Save model (LoRA adapters and merged model)
        save_medical_model(model, tokenizer)
        
        # Optional: Export to GGUF for mobile deployment
        print("\n=== Optional GGUF Export ===")
        export_gguf = input("Export GGUF model for mobile deployment? (y/n): ").lower().strip()
        if export_gguf == 'yes' or export_gguf == 'y':
            try:
                export_gguf_model(model, tokenizer)
                print("✅ GGUF export completed")
            except Exception as gguf_error:
                print(f"⚠️  GGUF export failed: {gguf_error}")
                print("You can run gguf_export.py separately later")
        
        # Optional: Upload to HuggingFace Hub
        print("\n=== Optional HuggingFace Upload ===")
        upload_hf = input("Upload to HuggingFace Hub? (y/n): ").lower().strip()
        if upload_hf == 'yes' or upload_hf == 'y':
            if UPLOAD_TO_HF and check_hf_auth():
                try:
                    upload_to_huggingface(model, tokenizer, repo_id=HF_REPO_ID, private=PRIVATE_REPO)
                    print("✅ HuggingFace upload completed")
                except Exception as hf_error:
                    print(f"⚠️  HuggingFace upload failed: {hf_error}")
                    print("You can run hf_upload.py separately later")
            else:
                print("⚠️  HuggingFace upload not configured or authentication failed")
                print("You can run hf_upload.py separately after configuring authentication")
        ...
        print("   - Run gguf_export.py for mobile deployment")
        print("   - Run hf_upload.py for HuggingFace Hub upload")
        print("   - Integrate with relay-responder-app")
