...
class ApiService {
  ...
  async callOpenRouterAPI(text: string): Promise<string> {
    try {
  ...
  async callOpenRouterVisionAPI(frames: string[], text: string = 'Analyze the following sequence of images and provide a concise, one-sentence summary of the situation.'): Promise<string> {
    try {
      const apiKey = process.env.EXPO_PUBLIC_OPENROUTER_API_KEY;
      if (!apiKey) {
        throw new Error('OpenRouter API key not found');
